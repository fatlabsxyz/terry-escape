...
const zkAlicia = new zklib(0, secretKeySample(0), [0,1,2,3].map(i => publicKeySample(i)), { mockProof: true });
/// BRENDA
...
async function flow() {
  ...
  const { proof: answerProof } = await zkAlicia.createAnswers([queryProof, queryProof, queryProof], action);
